...
export class CreateDetallePlanillaDto {
    ...
    @IsNumber()
    @IsNotEmpty()
    horasJornadaCompletaAcumuladas: number;

    @IsNumber()
    @IsNotEmpty()
    horasExtrasPuraAcumuladas: number;

    @IsNumber()
    @IsNotEmpty()
    horasIncompletasABolsa: number;
...
@Table('DETALLES_PLANILLA', { dto: CreateDetallePlanillaDto })
