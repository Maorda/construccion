...
export class SheetsRepository<T extends object> {
    ...
    // =========================================================================
    // API PÚBLICA DE LECTURA (ESTILO MONGOOSE)
    // =========================================================================
    ...
    async save(doc: SheetDocument<T>): Promise<SheetDocument<T>> {
        // 1. Asignación Temprana: Asegurar que el documento tenga su ID ANTES de procesar las cascadas.
        // Vital para que las @SubCollection puedan heredar la Foreign Key del padre en memoria.
        if (!doc[ROW_INDEX_SYMBOL]) {
            await this.applyAutogeneratedFields(doc);
        }

        // 2. Procesar relaciones en cascada (en memoria mutará las FKs y llamará a childRepo.save)
        await this.processCascadeSave(doc);

        // 3. Integración Transaccional
        if (this.unitOfWork.hasActiveTransaction()) {
            // Escudo anti-bucles: Evitar encolar el mismo documento múltiples veces si hay referencias circulares
            const isAlreadyQueued = this.unitOfWork.getPendingOperations().some(
                op => op.doc === doc
            );
            ...
            return doc; // Retorna rápido, el UOW hará el flush físico luego
        }

        // --- FLUJO DIRECTO (Sin transacción) ---
        const rowNumber = doc[ROW_INDEX_SYMBOL];
        if (!rowNumber) {
            // insertDocument es idempotente, no afectará si applyAutogeneratedFields ya se ejecutó
    ...
    private validateWithJoi(data: any): any {
        ...
        Object.keys(details).forEach(prop => {
            ...
            if (config.type === Number || config.type === 'number' || config.type === 'currency') validator = Joi.number();
            else if (config.type === Boolean || config.type === 'boolean') validator = Joi.boolean();
            else if (config.type === Date || config.type === 'date') validator = Joi.date();
            else if (config.type === 'array' || Array.isArray(config.type)) validator = Joi.array();
            else if (config.type === 'json' || typeof config.type === 'object') validator = Joi.any();
            else validator = Joi.string().allow('');
