...
export class SheetsRepository<T extends object> {
    ...
    async save(doc: SheetDocument<T>): Promise<SheetDocument<T>> {
        // 1. Autogenerar IDs si es necesario (Garantiza que el campo PK tenga valor aquí)
        ...
        // 🟢 Obtener de forma segura el valor de la clave primaria (Ej: 'id', 'dni', etc.)
        const pkField = this.metadataRegistry.getPrimaryKeyField(this.entityClass);
        const pkValue = doc[pkField];

        // 4. Integración Transaccional Robusta (UnitOfWork)
        if (this.unitOfWork.hasActiveTransaction()) {
            const pendingOps = this.unitOfWork.getPendingOperations();

            // 🔥 Corrección del Bug de Referencia: Buscamos por Identidad Lógica (Clase + PK)
            const existingOp = pendingOps.find(op =>
                op.entityClass === this.entityClass && String(op.pk) === String(pkValue)
            );

            if (!existingOp) {
                // Si no existe en la cola, determinamos si es un UPDATE o INSERT real
                ...
                this.unitOfWork.queueOperation({
                    ...
                    doc,
                    pk: pkValue // 🟢 Solución al Error 1: Pasamos la PK requerida por la interfaz
                });

                // Registramos de paso en el Identity Map para proteger el contexto
                this.unitOfWork.register(doc, pkValue, this.entityClass);
            } else {
                // 🔥 Solución al Error 3: Si ya estaba encolado como INSERT, no hacemos nada.
                // Dado que JavaScript pasa los objetos por referencia, cualquier cambio realizado 
                // en 'doc' se reflejará automáticamente en el payload cuando el UOW haga el commit.
                this.logger.debug(`[UOW] Registro [${this.sheetName}] con PK [${pkValue}] ya se encuentra encolado. Sincronizando mutaciones por referencia.`);
            }

            return doc;
        }

        // 5. Guardado físico directo si no hay transacción activa (Operación síncrona/inmediata)
    ...
    async delete(filter: FilterQuery<T>): Promise<boolean> {
        ...
        // 1. Procesar cascadas (Los deletes hijos se encolarán de forma recursiva en el mismo UOW)
        await this.processCascadeDelete(doc);

        // Mapeos de metadata necesarios
        const pkField = this.metadataRegistry.getPrimaryKeyField(this.entityClass);
        const pkValue = doc[pkField];
        ...
        // 2. Si el modelo usa Soft Delete, mutamos la propiedad de control de una vez
        // Esto asegura que si el documento se serializa, viaje con el flag en 'true' o 'deleted'
        if (deleteControlProp) {
            (doc as any)[deleteControlProp] = true;
        }

        // 3. Integración Transaccional Inteligente (UnitOfWork)
        if (this.unitOfWork.hasActiveTransaction()) {
            const pendingOps = this.unitOfWork.getPendingOperations();

            // Buscamos si este mismo registro ya tenía una acción previa en esta transacción
            const existingOpIndex = pendingOps.findIndex(op =>
                op.entityClass === this.entityClass && String(op.pk) === String(pkValue)
            );

            if (existingOpIndex !== -1) {
                const existingOp = pendingOps[existingOpIndex];

                // 🔥 CASO 1: EFECTO NETO CERO
                if (existingOp.type === 'INSERT') {
                    // Se creó y se borró antes de ir a Sheets. 
                    // Removemos el INSERT de la cola y no agregamos el DELETE. Mutación cancelada.
                    pendingOps.splice(existingOpIndex, 1);
                    this.logger.log(`[UOW] 🧼 Efecto Neto Cero detectado para [${this.sheetName}] PK [${pkValue}]. Operaciones removidas de la cola.`);
                    return true;
                }

                // 🔥 CASO 2: SOBREESCRITURA DE UPDATE
                if (existingOp.type === 'UPDATE') {
                    // Si ya se iba a actualizar pero ahora se va a borrar, el UPDATE pierde sentido.
                    pendingOps.splice(existingOpIndex, 1);
                }
            }

            // Encolamos la operación definitiva pasando el contrato completo
            this.unitOfWork.queueOperation({
                // Si es soft delete, para la Outbox esto es conceptualmente un UPDATE de celdas. 
                // Si es hard delete, la Outbox sabe que debe limpiar/vaciar la fila físicamente.
                type: deleteControlProp ? 'UPDATE' : 'DELETE',
                entityClass: this.entityClass,
                sheetName: this.sheetName,
                doc,
                pk: pkValue // 🟢 Solución: Inyectamos la clave primaria obligatoria
            });

            return true;
        }

        // 4. Guardado físico directo (Fuera de transacciones / Operación inmediata)
        if (deleteControlProp) {
            // Reutiliza tu método save que ya maneja la persistencia directa de propiedades mutadas
            await this.save(doc);
        } else if (rowIndex) {
            // Hard delete directo a la celda por la API v4
